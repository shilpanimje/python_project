...
```bash
(env) $ py.test tests/ --cov python_crud --cov-report term-missing
(env) $ flake8 python_crud/ tests/
```
...
1. Rename `python_crud` directory with the name of your application.
2. Find all occurrences of `python_crud` in your application and rename them with
